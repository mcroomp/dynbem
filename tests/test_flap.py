"""Tests for the passive feathering DOF model with servo-flaps.

Covers:
  1. Rigid blade (no passive_feathering) is unchanged -- all existing models still work.
  2. PassiveFeatheringProperties round-trips through rotor YAML loader (test_flap_rotor).
  3. PassiveFeatheringProperties round-trips through _to_rust.
...
from dynbem.rotor_definition import (
    load, PassiveFeatheringProperties, ServoFlapProperties, RotorDefinition,
...
def make_minimal_defn(with_passive_feathering=False, with_servo=False, damper=5.0):
    blade = BladeGeometry(
    ...
    if with_passive_feathering:
        ...
        passive_feathering = PassiveFeatheringProperties(
            ...
            ac_offset_m=0.0, servoflaps=servoflaps,
        )
    return RotorDefinition(
        blade=blade, airfoil=airfoil, control=control,
        passive_feathering=passive_feathering, name="test", description="",
    )
...
# ---------------------------------------------------------------------------
# 1. Rigid blade is unchanged
# ---------------------------------------------------------------------------

def test_rigid_blade_pitt_peters_runs():
    defn = make_minimal_defn(with_passive_feathering=False)
...
def test_feathering_rotor_yaml_loads():
    ...
    assert defn.passive_feathering is not None, "test_flap_rotor should have passive_feathering: section"
    fp = defn.passive_feathering
    assert fp.I_theta_kgm2 == pytest.approx(0.0012)
    assert fp.damper_Nms_per_rad == pytest.approx(0.5)
    assert fp.ac_offset_m == pytest.approx(0.0)
    assert fp.servoflaps is not None
    assert fp.servoflaps.C_M_delta_per_rad == pytest.approx(-1.5)
    assert fp.servoflaps.r_inner_m == pytest.approx(1.2)
    assert fp.servoflaps.r_outer_m == pytest.approx(2.5)


# ---------------------------------------------------------------------------
# 3. FeatheringProperties round-trips through _to_rust
# ---------------------------------------------------------------------------

def test_feathering_rust_roundtrip():
    fp = PassiveFeatheringProperties(
        I_theta_kgm2=0.002, damper_Nms_per_rad=3.0, ac_offset_m=0.005,
        servoflaps=ServoFlapProperties(C_M_delta_per_rad=-1.2, r_inner_m=0.4, r_outer_m=1.1),
    )
    rust = fp._to_rust()
    assert isinstance(rust, _RustPassiveFeatheringProperties)
...
# ---------------------------------------------------------------------------
# 4. Servo command changes hub moment vs. rigid blade
# ---------------------------------------------------------------------------
...
def test_servo_command_changes_hub_moment():
    ...
    assert mx_feat != pytest.approx(mx_rigid, abs=1e-3), (
        f"feathering model should differ from rigid: feat={mx_feat:.4f} rigid={mx_rigid:.4f}"
    )
...
def test_authority_increases_with_less_damping():
    inputs = make_inputs(tilt_lat=0.1, v_wind=3.0)
    defn_r = make_minimal_defn(with_passive_feathering=False)
    model_r = dynbem.create_aero(defn_r, model="pitt_peters")
    res_r, _ = model_r.compute_forces(inputs, model_r.initial_rotor_state())
    mx_rigid = float(res_r.M_orbital[0])

    results = {}
    for damper, label in [(1.0, "lo"), (10.0, "hi")]:
        defn = make_minimal_defn(with_passive_feathering=True, with_servo=True, damper=damper)
        model = dynbem.create_aero(defn, model="pitt_peters")
        res, _ = model.compute_forces(inputs, model.initial_rotor_state())
        results[label] = abs(float(res.M_orbital[0]) - mx_rigid)
...
# ---------------------------------------------------------------------------
# 6. OyeBEMModel runs with feathering
# ---------------------------------------------------------------------------

def test_oye_with_feathering_runs():
    defn = make_minimal_defn(with_passive_feathering=True, with_servo=True)
...
# ---------------------------------------------------------------------------
# 8. Full beaupoil feathering rotor runs compute_forces
# ---------------------------------------------------------------------------
