...
from tests.helpers import make_oye, make_pitt_peters, pp_state
...
class TestOyeRotorState:
    def test_zeros_factory_returns_correct_shape(self):
        s = OyeRotorState.zeros(n_elements=10)
...
class TestOyeLag:
    ...
    def test_step_response_has_lag(self, oye_model, defn):
        ...
        # Step up collective by 2 deg.  Take ONE step.
        inp2 = RotorInputs(
...
class TestOyeEnvelopeStability:
    ...
    def test_descent_with_sidewind_converges(self, defn):
        ...
        I_ode = float(defn_b.autorotation.I_ode_kgm2 or 1.0)
        ...
        state = OyeRotorState.zeros(defn_b.blade.n_elements)
        omega = 20.0
        v_along = 0.0
        col = 0.0
        int_v = 0.0
        ...
        assert 20.0 < omega < 100.0, f"omega={omega}"
